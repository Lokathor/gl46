...
/// Used by fns to invert a condition
/// The tokens are removed if the input is `true`
/// With the pattern used by `fns`, this should be invoked as
/// `not!($($($NAME)? true)? TOKENS)`
macro_rules! not {
  ((true) $($e:tt)*) => {};
  (() $($e:tt)*) => {$($e)*};
}
    ...
    impl GlFns {
      unsafe fn _load_from(f: &dyn Fn(*const u8) -> *const c_void) -> Result<Self, &'static str> {
        Ok(Self{
          $(
            $name: {
              // Generate the internal OpenGL function name
              let s = concat!("gl", stringify!($name), "\0");
              let ptr = Self::ptr_filter(f(s.as_ptr()));
              // If a builtin function is missing, return the error
              not!(($($($ext)? true)?) let ptr = ptr.ok_or(s)?;);
    ...
    impl GlFns {
      $(
        ...
        pub $(unsafe $($unsafe)?)? fn $name(&self, $($arg : $t),*) $(-> $ret)* {
          let f = self.$name;
          // We need to unwrap the extension functions
          $($($ext)?
            let f = match f {
              Some(f) => f,
